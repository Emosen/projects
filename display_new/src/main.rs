...
impl fmt::Display for Structure {
    // This trait requires `fmt` which this exact signature.
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        // which is very simiar to `println!`
        write!(f, "{}", self.0)
    }
}
...
// Define a structure where the fields are nameable for comparision
#[derive(Debug)]
struct Point2D {
    x: f64,
    y: f64,
...
impl fmt::Display for Point2D {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result { 
        // Customize so only `x` and `y` are dnoted.
